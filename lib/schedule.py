...
class Job(object):
    ...
    def _schedule_next_run(self):
        ...
        if self.unit == 'minutes':
            ...
            f = 60 * 60 * 24 * 7
        self.period = int(f * self.interval)
        self.next_run = now()

        if self.at_time is not None:
            tm = list(time.localtime(int(self.next_run)))
            ...
            tm[4] = self.at_time[1]
            tm[5] = 0
            self.next_run = int(time.mktime(tuple(tm)))

            # if the target time is not passed for today then the task may be
            # run today
            if not passed_today:
                self.next_run -= int(self.period)
        ...
        if self.start_day is not None:
            ...
            wd = time.localtime(int(self.next_run))[6]
            assert self.start_day in weekdays
            weekday = weekdays.index(self.start_day)
            days_ahead = weekday - wd
            if days_ahead <= 0:  # Target day already happened this week
                days_ahead += 7
            self.next_run += int(days_ahead * 60 * 60 * 24 - self.period)
